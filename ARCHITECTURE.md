...
│   └── src/main/java/cn/jason31416/multiauth/api/
...
├── core/                       # 核心插件模块
│   └── src/main/java/cn/jason31416/multiauth/
...
| 命令 | 权限 | 说明 |
|---|---|---|
| `/multiauth changepass <player> <newpass>` | `multiauth.admin` | 强制修改指定玩家密码 |
| `/multiauth unregister <player>` | `multiauth.admin` | 删除玩家账号 |
| `/multiauth setuuid <player> [uuid]` | `multiauth.admin` | 手动设置玩家 UUID |
| `/multiauth reload` | `multiauth.admin` | 重载配置文件 |
| `/multiauth recover` | `multiauth.admin` | 从备份恢复账号数据 |
...
| **TAB** | 可选，注册 `%multiauth-auth-tag%` 占位符，显示玩家认证方式前缀 |
