...
public class MessageLoader {
    ...
    public static void initialize(){
        File langfolder = new File(MultiAuth.getInstance().getDataDirectory(), "lang");
        if(!langfolder.exists()) {
            langfolder.mkdir();
            List<String> supported = List.of("en-us", "zh-cn");
            supported.forEach(lang -> {
                File langfile = new File(MultiAuth.getInstance().getDataDirectory(), "lang/"+ lang+".yml");
                try (InputStream inputStream = MultiAuth.class.getClassLoader().getResourceAsStream("lang/"+ lang+".yml"); OutputStream outputStream = new FileOutputStream(langfile)) {
        ...
        File lang = new File(MultiAuth.getInstance().getDataDirectory(), "lang/"+ Config.getString("lang")+".yml");
