...
package cn.jason31416.multiauth.injection.accessor;
...
public class Accessor {
     ...
     */
    public Constructor<?> findFirstConstructors(boolean declared, Function<Constructor<?>, Boolean> function, String exceptionMessage) {
        List<Constructor<?>> elements = getElements(declared ? classHandle.getDeclaredConstructors() : classHandle.getConstructors(), function);
        if (elements.isEmpty()) throw new RuntimeException(exceptionMessage);
        return elements.get(0);
    }
     ...
     * 使用给定的类型检索第一次出现的 Field
     */
    public Constructor<?> findFirstConstructorByParameterTypes(boolean declared, Type[] types) {
